...
def poetry_version_file = "lib/poetry/version.rb"
...
namespace :version do
  desc "Fail unless the pushed tag (GITHUB_REF_NAME) is v<Poetry::VERSION> (the release guard)"
  task :verify_tag do
    tag = ENV.fetch("GITHUB_REF_NAME") { abort "version:verify_tag reads GITHUB_REF_NAME (the pushed tag)" }
    expected = "v#{poetry_gem_version}"
    abort "tag #{tag} does not match Poetry::VERSION (#{expected})" unless tag == expected

    puts "tag #{tag} matches Poetry::VERSION"
  end

  desc "Set Poetry::VERSION: rake \"version:bump[X.Y.Z]\""
  task :bump, [:version] do |_, args|
    ...
    puts "bumped Poetry::VERSION to #{version}; commit, then tag v#{version}"
  end
end
