...
- PART `navigation-menu-trigger` - The disclosure button opening its panel | states: data-popup-open (its panel is open (written with aria-expanded - the chevron rotation hook)); data-open (its panel is open (the controller writes both vocabularies)); data-closed (its panel is closed (written after the first close)); data-disabled (disabled: true - inert (native disabled rides along; hover and click never open the panel, the arrows step over it))
