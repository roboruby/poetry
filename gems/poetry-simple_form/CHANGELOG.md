...
- The gem is developed in the Poetry monorepo, roboruby/poetry, under `gems/poetry-simple_form`; the gemspec's source, changelog and issue links point there, and its file list no longer needs git on the machine (a path gem inside a slim image lists the same files).
